---
title: "University of Notre Dame WBB Travel Stress Metric"
format: gfm
execute:
  warning: false
  message: false
...
We loaded the tooling needed for data preparation, standardization, interaction generation, and regression modeling, then read the final cleaned dataset.
...
We transformed raw fields so that “higher = more stress” and created additional indicators needed for the index.
...
We isolated the core variables that represent travel/schedule stress and store them in a single table.
...
We calculated the data-derived weights for primary stress factors by regressing scoring margin on the standardized predictors. We converted absolute standardized coefficients into normalized weights.
...
Instead of manually assigning interaction weights, we estimated a regression including both primary and interaction terms, then derive weights from the interaction coefficients.
